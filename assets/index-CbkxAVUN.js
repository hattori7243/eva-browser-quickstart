...
registerProcessor("${Ih}", EvaAudioCaptureProcessor);
`;function zh(e={}){return Bh(e)}function Bh(e={},t={}){return new Vh(e,t)}var Vh=class{constructor(e,t){this.options=e,this.dependencies=t}options;dependencies;generation=0;session;async start(){await(this.session??this.createSession()).startPromise}frames(e){let t=this.session;return t===void 0?Gh(new Ah(`Browser audio input is not started`,{fatal:!0})):Uh(e)?this.consumeAbortedSession(t):t.consumerActive?Gh(new Ah(`Browser audio input already has an active consumer`,{fatal:!0})):(t.consumerActive=!0,this.consumeSession(t,e))}async*consumeAbortedSession(e){await this.teardown(e)}async*consumeSession(e,t){let n,r,i=new Promise(e=>{n=e}),a=()=>{r=this.teardown(e),n?.()};t?.addEventListener(`abort`,a,{once:!0});try{if(await Promise.race([e.startPromise,i]),e.failure!==void 0)throw e.failure;if(Uh(t)||e.cancelled)return;for(;!Uh(t);){if(e.failure!==void 0)throw e.failure;if(e.cancelled)return;let t=await e.queue.next();if(t.done)return;yield t.value}}finally{t?.removeEventListener(`abort`,a),e.consumerActive=!1,await(r??this.teardown(e))}}async stop(){let e=this.session;e!==void 0&&await this.teardown(e)}createSession(){let e,t,n=new Promise((e,n)=>{t=n}),r=new Kh(qh(this.options.maxBufferedChunks),t=>{e.failure=t,this.teardown(e)});return e={generation:++this.generation,queue:r,encoder:new Nh,startPromise:Promise.resolve(),consumerActive:!1,cancelled:!1,rejectCancellation:t,stream:void 0,context:void 0,sourceNode:void 0,workletNode:void 0,sampleRate:void 0,ownedWorkletUrl:void 0},this.session=e,e.startPromise=Promise.race([this.startSession(e),n]),e.startPromise.catch(()=>{}),e}async startSession(e){try{let t=await Yh(this.dependencies).getUserMedia({audio:{autoGainControl:this.options.autoGainControl??!0,echoCancellation:this.options.echoCancellation??!0,noiseSuppression:this.options.noiseSuppression??!0}});if(!this.isCurrent(e))throw Wh(t),Hh();e.stream=t;let n=Xh(this.dependencies);e.context=n,e.sampleRate=n.sampleRate,this.throwIfNotCurrent(e);let r=this.dependencies.workletModuleUrl??Qh(this.dependencies).create(Rh);this.dependencies.workletModuleUrl===void 0&&(e.ownedWorkletUrl=r),await n.audioWorklet.addModule(r),this.throwIfNotCurrent(e);let i=n.createMediaStreamSource(t),a=Zh(this.dependencies,n);e.sourceNode=i,e.workletNode=a,a.port.onmessage=t=>this.handleSamples(e,t.data),i.connect(a),a.connect(n.destination),await n.resume(),this.throwIfNotCurrent(e)}catch(t){let n=e.cancelled||this.session!==e;throw await this.teardown(e),n?Hh():jh(t,{message:`Browser audio input failed`})}}throwIfNotCurrent(e){if(!this.isCurrent(e))throw Hh()}isCurrent(e){return!e.cancelled&&this.session===e}handleSamples(e,t){if(!(!this.isCurrent(e)||!Jh(t)||e.context===void 0||e.sampleRate===void 0))try{if(e.context.sampleRate!==e.sampleRate)throw RangeError(`AudioContext sampleRate changed during capture`);for(let n of e.encoder.push(t,e.sampleRate))e.queue.push({data:n,sampleRate:e.sampleRate,channels:1,format:`pcm_s16le`})}catch(t){let n=jh(t,{message:`Browser audio input encoding failed`});e.failure=n,e.queue.fail(n),this.teardown(e)}}teardown(e){if(e.teardownPromise!==void 0)return e.teardownPromise;e.cancelled=!0,e.rejectCancellation?.(Hh()),e.rejectCancellation=void 0,e.queue.close(),this.session===e&&(this.session=void 0);let t=e.workletNode,n=e.sourceNode,r=e.stream,i=e.context,a=e.ownedWorkletUrl;return e.workletNode=void 0,e.sourceNode=void 0,e.stream=void 0,e.context=void 0,e.sampleRate=void 0,e.ownedWorkletUrl=void 0,e.teardownPromise=(async()=>{t!==void 0&&(t.port.onmessage=null,t.disconnect()),n?.disconnect(),r!==void 0&&Wh(r),await i?.close(),a!==void 0&&Qh(this.dependencies).revoke(a)})(),e.teardownPromise}};function Hh(){return new Ah(`Browser audio input stopped`,{fatal:!0})}function Uh(e){return e?.aborted===!0}function Wh(e){for(let t of e.getAudioTracks())t.stop()}async function*Gh(e){throw e}var Kh=class{constructor(e,t){this.limit=e,this.onOverflow=t}limit;onOverflow;items=[];waiters=[];closed=!1;error;push(e){if(this.closed||this.error!==void 0)return;let t=this.waiters.shift();if(t!==void 0){t.resolve({done:!1,value:e});return}if(this.items.length>=this.limit){let e=new Ah(`Browser audio input buffer overflow`,{fatal:!0});this.error=e,this.items.length=0;for(let e of this.waiters.splice(0))e.reject(this.error);this.onOverflow(e);return}this.items.push(e)}fail(e){if(!(this.closed||this.error!==void 0)){this.error=e,this.items.length=0;for(let t of this.waiters.splice(0))t.reject(e)}}next(){if(this.error!==void 0)return Promise.reject(this.error);let e=this.items.shift();return e===void 0?this.closed?Promise.resolve({done:!0,value:void 0}):new Promise((e,t)=>this.waiters.push({resolve:e,reject:t})):Promise.resolve({done:!1,value:e})}close(){if(!this.closed&&(this.closed=!0,this.error===void 0))for(let e of this.waiters.splice(0))e.resolve({done:!0,value:void 0})}};function qh(e){if(e===void 0)return Lh;if(!Number.isInteger(e)||e<=0)throw new Ah(`Browser audio input maxBufferedChunks must be a positive integer`,{fatal:!0});return e}function Jh(e){return Array.isArray(e)&&e.length>0&&e.every(e=>e instanceof Float32Array)}function Yh(e){let t=e.mediaDevices??globalThis.navigator?.mediaDevices;if(t===void 0)throw new Ah(`Browser media devices unavailable`,{fatal:!0});return t}function Xh(e){if(e.audioContextFactory!==void 0)return e.audioContextFactory();if(typeof AudioContext>`u`)throw new Ah(`Browser AudioContext unavailable`,{fatal:!0});return new AudioContext({latencyHint:`interactive`})}function Zh(e,t){return e.audioWorkletNodeFactory===void 0?new AudioWorkletNode(t,Ih):e.audioWorkletNodeFactory(t,Ih)}function Qh(e){return e.objectUrl??{create(e){return URL.createObjectURL(new Blob([e],{type:`text/javascript`}))},revoke(e){URL.revokeObjectURL(e)}}}function $h(){return eg()}function eg(e={}){return new tg(e)}var tg=class{constructor(e){this.dependencies=e}dependencies;context;scheduledTime=0;active=new Set;drainWaiters=new Set;generation=0;stopped=!1;stopPromise;async enqueue(e){if(this.stopped)throw new Ah(`Browser audio output stopped`,{fatal:!0});if(e.format!==`pcm_s16le`)throw new Ah(`Browser audio output requires pcm_s16le`,{fatal:!0});let t=this.generation,n;try{if(n=this.context??this.createContext(),await n.resume(),!this.isCurrent(t,n))return;let r=Math.floor(e.data.byteLength/2/e.channels);if(r===0)return;let i=n.createBuffer(1,r,e.sampleRate);ng(e,i.getChannelData(0));let a=n.createBufferSource();a.buffer=i,a.connect(n.destination),a.onended=()=>{this.active.delete(a),this.resolveDrainIfIdle()};let o=Math.max(n.currentTime,this.scheduledTime);this.active.add(a);try{a.start(o),this.scheduledTime=o+i.duration}catch(e){throw a.onended=null,this.active.delete(a),this.resolveDrainIfIdle(),e}}catch(e){if(n!==void 0&&!this.isCurrent(t,n))return;throw jh(e,{message:`Browser audio output failed`})}}flush(){this.generation+=1;let e=this.context,t=[...this.active];this.active.clear();let n;for(let e of t){e.onended=null;try{e.stop(0)}catch(e){n??=e}}if(this.scheduledTime=e?.currentTime??0,this.resolveDrainIfIdle(),n!==void 0)throw jh(n,{message:`Browser audio output flush failed`})}drain(){return this.active.size===0?Promise.resolve():new Promise(e=>this.drainWaiters.add(e))}stop(){return this.stopPromise??=this.stopOnce(),this.stopPromise}createContext(){if(this.dependencies.audioContextFactory!==void 0)return this.context=this.dependencies.audioContextFactory(),this.context;if(typeof AudioContext>`u`)throw new Ah(`Browser AudioContext unavailable`,{fatal:!0});return this.context=new AudioContext({latencyHint:`interactive`}),this.context}resolveDrainIfIdle(){if(this.active.size===0){for(let e of this.drainWaiters)e();this.drainWaiters.clear()}}isCurrent(e,t){return!this.stopped&&e===this.generation&&t===this.context}async stopOnce(){this.stopped=!0;let e;try{this.flush()}catch(t){e=t}let t=this.context;this.context=void 0,this.scheduledTime=0;try{await t?.close()}catch(t){e??=t}if(e!==void 0)throw jh(e,{message:`Browser audio output stop failed`})}};function ng(e,t){let n=new DataView(e.data.buffer,e.data.byteOffset,e.data.byteLength);for(let r=0;r<t.length;r+=1){let i=0;for(let t=0;t<e.channels;t+=1){let a=(r*e.channels+t)*2;i+=n.getInt16(a,!0)/32768}t[r]=i/e.channels}}function rg(e={}){return ig(e)}function ig(e={},t={}){return og(e),new ag(e,t)}var ag=class{constructor(e,t){this.options=e,this.dependencies=t}options;dependencies;generation=0;active;pendingStart;pendingCapture;teardownPromise;start(e){if(e.aborted)return Promise.reject(hg());if(this.active!==void 0||this.pendingStart!==void 0||this.teardownPromise!==void 0)return Promise.reject(new Ah(`Browser camera session already exists`,{fatal:!0}));let t=++this.generation,n=pg(),r={generation:t,cancelled:!1,cancel:n.cancel,cancellation:n.promise,underlying:Promise.resolve()};return this.pendingStart=r,r.underlying=this.acquire(r).finally(()=>{this.pendingStart===r&&(this.pendingStart=void 0)}),r.underlying.catch(()=>{}),mg(r.underlying,e,()=>this.cancelStart(r),r.cancellation)}capture(e){if(e.aborted)return Promise.reject(hg());let t=this.active;if(t===void 0)return Promise.reject(new Ah(`Browser camera is not started`,{fatal:!0}));if(this.pendingCapture!==void 0)return Promise.reject(new Ah(`Browser camera capture is already in progress`,{fatal:!0}));let n=pg(),r={generation:t.generation,cancelled:!1,cancel:n.cancel,cancellation:n.promise,underlying:Promise.resolve(gg())};return this.pendingCapture=r,r.underlying=this.captureFrame(t,r).finally(()=>{this.pendingCapture===r&&(this.pendingCapture=void 0)}),r.underlying.catch(()=>{}),mg(r.underlying,e,()=>this.cancelCapture(r),r.cancellation)}stop(){if(this.teardownPromise!==void 0)return this.teardownPromise;let e=this.pendingStart,t=this.pendingCapture,n=this.active;this.generation+=1,e!==void 0&&this.cancelStart(e),t!==void 0&&this.cancelCapture(t),this.active=void 0,n!==void 0&&dg(n);let r=(async()=>{await Promise.allSettled([e?.underlying??Promise.resolve(),t?.underlying??Promise.resolve()]);let n=this.active;n!==void 0&&(this.active=void 0,dg(n))})();return this.teardownPromise=r.finally(()=>{(this.teardownPromise===r||this.teardownPromise!==void 0)&&(this.teardownPromise=void 0)}),this.teardownPromise}async acquire(e){let t=await sg(this.dependencies).getUserMedia({audio:!1,video:this.options.video??!0});if(!this.isCurrentStart(e))throw fg(t),hg();let n=cg(this.dependencies);n.srcObject=t;try{if(await n.play(),await ug(this.dependencies)(n),!this.isCurrentStart(e)||n.videoWidth<=0||n.videoHeight<=0)throw hg();this.active={generation:e.generation,stream:t,video:n}}catch(e){throw n.pause(),n.srcObject=null,fg(t),e}}async captureFrame(e,t){let n=e.video.videoWidth,r=e.video.videoHeight;if(n<=0||r<=0)throw new Ah(`Browser camera frame is not ready`,{fatal:!0});let i=lg(this.dependencies);i.width=n,i.height=r,i.drawImage(e.video,n,r);let a=this.options.mimeType??`image/png`,o=await i.encode(a,a===`image/jpeg`?this.options.jpegQuality:void 0);if(!this.isCurrentCapture(e,t))throw hg();let s=new Uint8Array(await o.arrayBuffer());if(!this.isCurrentCapture(e,t))throw hg();let c=o.type||a;if(s.byteLength===0||c!==`image/png`&&c!==`image/jpeg`)throw new Ah(`Browser camera produced invalid image data`,{fatal:!0});return{data:s,mimeType:c,width:n,height:r}}cancelStart(e){e.cancelled||(e.cancelled=!0,e.cancel())}cancelCapture(e){e.cancelled||(e.cancelled=!0,e.cancel())}isCurrentStart(e){return this.pendingStart===e&&!e.cancelled&&e.generation===this.generation}isCurrentCapture(e,t){return this.active===e&&this.pendingCapture===t&&!t.cancelled&&t.generation===this.generation}};function og(e){if(e.jpegQuality!==void 0&&(!Number.isFinite(e.jpegQuality)||e.jpegQuality<0||e.jpegQuality>1))throw new Ah(`Browser camera jpegQuality must be between 0 and 1`,{fatal:!0})}function sg(e){let t=e.mediaDevices??navigator.mediaDevices;if(t===void 0)throw new Ah(`Browser camera mediaDevices is unavailable`,{fatal:!0});return t}function cg(e){if(e.createVideo!==void 0)return e.createVideo();let t=document.createElement(`video`);return t.autoplay=!0,t.muted=!0,t.playsInline=!0,t}function lg(e){if(e.createCanvas!==void 0)return e.createCanvas();let t=document.createElement(`canvas`);return{get width(){return t.width},set width(e){t.width=e},get height(){return t.height},set height(e){t.height=e},drawImage(e,n,r){let i=t.getContext(`2d`);if(i===null)throw new Ah(`Browser camera canvas 2D context is unavailable`,{fatal:!0});i.drawImage(e,0,0,n,r)},encode(e,n){return new Promise((r,i)=>{t.toBlob(e=>{e===null?i(new Ah(`Browser camera image encoding failed`,{fatal:!0})):r(e)},e,n)})}}}function ug(e){return e.waitForVideoFrame===void 0?async e=>{if(e.videoWidth>0&&e.videoHeight>0)return;let t=e;await new Promise((e,n)=>{let r=()=>{a(),e()},i=()=>{a(),n(new Ah(`Browser camera video failed`,{fatal:!0}))},a=()=>{t.removeEventListener(`loadeddata`,r),t.removeEventListener(`canplay`,r),t.removeEventListener(`error`,i)};t.addEventListener(`loadeddata`,r,{once:!0}),t.addEventListener(`canplay`,r,{once:!0}),t.addEventListener(`error`,i,{once:!0})})}:e.waitForVideoFrame}function dg(e){e.video.pause(),e.video.srcObject=null,fg(e.stream)}function fg(e){for(let t of e.getVideoTracks())t.stop()}function pg(){let e,t=new Promise((t,n)=>{e=()=>n(hg())});return t.catch(()=>{}),{promise:t,cancel:e}}function mg(e,t,n,r){return new Promise((i,a)=>{let o=()=>{n(),a(hg())};t.addEventListener(`abort`,o,{once:!0}),Promise.race([e,r]).then(i,a).finally(()=>{t.removeEventListener(`abort`,o)}).catch(()=>{})})}function hg(){return new DOMException(`Browser camera operation aborted`,`AbortError`)}function gg(){return{data:new Uint8Array([0]),mimeType:`image/png`,width:1,height:1}}function _g(){return Mh()}var vg=[{definition:{name:`show_current_time`,description:`当用户询问当前时间时，查询当前页面所在设备的本地时间。`},handler(e,t){if(t.signal.aborted)return{ok:!1,message:`Command cancelled`};let n=new Date;return{ok:!0,message:`当前本地时间是 ${n.toLocaleTimeString()}`,data:{isoTime:n.toISOString(),localTime:n.toLocaleString(),timeZone:Intl.DateTimeFormat().resolvedOptions().timeZone}}}},{definition:{name:`set_page_theme`,description:`当用户要求切换页面外观时，将当前页面切换为指定的明暗主题。`,parameters:[{name:`theme`,description:`要应用的页面主题。`,type:`string`,required:!0,enum:[`light`,`dark`],example:`dark`}]},handler(e,t){if(t.signal.aborted)return{ok:!1,message:`Command cancelled`};let n=e.arguments.theme;return n!==`light`&&n!==`dark`?{ok:!1,message:`Unsupported page theme`}:(yg(n),{ok:!0,message:`页面主题已切换为 ${n}`,data:{theme:n}})}}];function yg(e){document.documentElement.dataset.theme=e,document.documentElement.style.colorScheme=e}async function bg(e){let t={input:zh({maxBufferedChunks:64,echoCancellation:!0,noiseSuppression:!0,autoGainControl:!0}),output:$h(),aec:_g(),camera:rg({video:{facingMode:{ideal:`user`}},mimeType:`image/png`})},n=Th({apiKey:e.apiKey,asr:{model:`fun_asr`,sampleRate:48e3},llm:{model:`doubao-seed-2-0-mini-nothink`},tts:{model:`cosyvoice_tts`,voice:`longjielidou_v3`,sampleRate:48e3},vad:{sensitivity:.7,silenceThresholdMs:400},transports:t,history:{maxTurns:10},camera:{captureTimeoutMs:1500},emotion:{enabled:!0,labels:[`happy`,`sad`]},commands:{registrations:vg,maxCallsPerTurn:3},systemPrompt:[`你是一个简洁、自然的语音助手。`,`只有当前用户消息实际包含图片时，才可以描述当前看到的画面。`,`当前用户消息没有图片时，不得把历史中的视觉描述说成实时观察；`,`如需引用，只能明确说明那是之前看到的内容，并说明当前没有新的画面。`].join(``),greeting:{mode:`static`,text:`你好`}}),r=n.onEvent(t=>e.onEvent(t,n));try{return await n.setAudioInputEnabled(e.audioInputEnabled),await n.setCameraCaptureEnabled(e.cameraEnabled),await n.setTtsEnabled(e.ttsEnabled),await n.start(),{agent:n,unsubscribe:r}}catch(e){throw r(),e}}var xg=Zg(void 0),Sg,Cg,wg,Tg=!0,Eg=!0,Dg=!1,Og=!1,kg,Ag,jg=$(`start`),Mg=$(`stop`),Ng=$(`audio-input`),Pg=$(`tts`),Fg=$(`camera`),Ig=$(`text-form`),Lg=$(`text`),Rg=Ig.querySelector(`button[type=submit]`),zg=$(`clear-log`),Bg=$(`api-key-dialog`),Vg=$(`api-key-form`),Hg=$(`api-key-input`),Ug=$(`api-key-error`),Wg=$(`api-key-cancel`),Gg=$(`voice-stage`),Kg=$(`conversation-empty`),qg=$(`call-duration`);jg.addEventListener(`click`,()=>void Jg()),Mg.addEventListener(`click`,()=>void Qg()),Ng.addEventListener(`click`,()=>void $g()),Pg.addEventListener(`click`,()=>void e_()),Fg.addEventListener(`click`,()=>void t_()),Ig.addEventListener(`submit`,e=>void n_(e)),zg.addEventListener(`click`,()=>{$(`activity-log`).value=``}),Vg.addEventListener(`submit`,e=>{if(e.preventDefault(),Zg(Hg.value)===void 0){Ug.textContent=`请输入有效的 Gateway AK`,Hg.focus();return}Bg.close(`confirm`)}),Wg.addEventListener(`click`,()=>Bg.close(`cancel`));async function Jg(){if(Cg===void 0){o_(!0);try{let e=await Yg();if(e===void 0){Q(`status`,`未启动；未提供 AK`),p_(`idle`,`准备好聊聊了吗？`,`启动后直接开口，EVA 会实时听见并回应你。`);return}s_(!0),Q(`status`,`启动中…`),p_(`starting`,`正在连接 EVA`,`请稍候，我们正在准备麦克风和语音通道。`),Q(`microphone`,Tg?`请求权限中…`:`已禁用`);let t=await bg({apiKey:e,audioInputEnabled:Tg,ttsEnabled:Eg,cameraEnabled:Dg,onEvent:r_});Cg=t.agent,wg=t.unsubscribe,Q(`status`,`已接通`),p_(Tg?`ready`:`idle`,Tg?`通话已接通`:`语音输入已关闭`,Tg?`直接开口即可，EVA 在听。`:`打开麦克风，或使用下方文字输入。`),Q(`microphone`,Tg?`已连接`:`已禁用`),Q(`camera-status`,Dg?`持续连接；说话时采一张图`:`默认关闭`),c_(!0),Rg.disabled=!1,s_(!1)}catch(e){Q(`status`,`启动失败`),p_(`error`,`连接没有成功`,`请检查 AK、网络与设备权限后重试。`),Q(`microphone`,`不可用`),Q(`error`,e instanceof Error?e.message:String(e)),wg?.(),wg=void 0,Cg=void 0,c_(!1),s_(!1)}finally{o_(!1),jg.disabled=Cg!==void 0}}}async function Yg(){if(xg!==void 0)return xg;if(Sg!==void 0)return Sg;let e=await Xg();return e!==void 0&&(Sg=e),e}function Xg(){return Hg.value=``,Ug.textContent=``,Bg.returnValue=``,new Promise(e=>{Bg.addEventListener(`close`,()=>{let t=Bg.returnValue===`confirm`?Zg(Hg.value):void 0;Hg.value=``,e(t)},{once:!0}),Bg.showModal(),Hg.focus()})}function Zg(e){let t=e?.trim();return t===void 0||t.length===0||t.startsWith(`replace-`)?void 0:t}async function Qg(){let e=Cg;if(e!==void 0){Cg=void 0,Rg.disabled=!0,Mg.disabled=!0,jg.disabled=!0,u_(),s_(!0),Q(`status`,`停止中…`),p_(`processing`,`正在结束会话`,`EVA 正在释放本次会话使用的设备。`);try{await e.stop(),a_(e),Q(`status`,`通话已结束；再次启动会创建新会话`),p_(`idle`,`本次对话已结束`,`再次启动会创建一个新的会话。`),Q(`microphone`,`已释放`),Q(`camera-status`,`已释放`)}catch(e){Q(`error`,e instanceof Error?e.message:String(e))}finally{wg?.(),wg=void 0,c_(!1),jg.disabled=!1}}}async function $g(){if(Ng.disabled)return;let e=!Tg;Ng.disabled=!0;try{await Cg?.setAudioInputEnabled(e),Tg=e,f_(Ng,Tg),Cg!==void 0&&p_(Tg?`ready`:`idle`,Tg?`我在听`:`语音输入已关闭`,Tg?`继续说，EVA 会自然接着聊。`:`打开麦克风，或使用下方文字输入。`),Q(`microphone`,Tg?Cg===void 0?`启动时启用`:`已连接`:Cg===void 0?`启动时禁用`:`已释放`)}catch(e){Q(`error`,e instanceof Error?e.message:String(e))}finally{Ng.disabled=Og}}async function e_(){if(Pg.disabled)return;let e=!Eg;Pg.disabled=!0;try{await Cg?.setTtsEnabled(e),Eg=e,f_(Pg,Eg)}catch(e){Q(`error`,e instanceof Error?e.message:String(e))}finally{Pg.disabled=Og}}async function t_(){if(Fg.disabled)return;let e=!Dg;Fg.disabled=!0;try{await Cg?.setCameraCaptureEnabled(e),Dg=e,f_(Fg,Dg),Q(`camera-status`,Dg?Cg===void 0?`启动时启用`:`持续连接；说话时采一张图`:Cg===void 0?`启动时关闭`:`已释放`)}catch(e){Q(`error`,e instanceof Error?e.message:String(e))}finally{Fg.disabled=Og}}async function n_(e){e.preventDefault();let t=Lg.value.trim(),n=Cg;if(!(n===void 0||t.length===0)){Lg.value=``;try{await n.submitText(t,{metadata:{channel:`ts-browser-demo`}})}catch(e){Q(`error`,e instanceof Error?e.message:String(e))}}}function r_(e,t){switch(e.type){case`speech.started`:Q(`microphone`,`检测到说话`),p_(`listening`,`我在听`,`继续说，停顿后 EVA 会自动理解并回应。`);return;case`image.captured`:Q(`camera-status`,`${e.image.mimeType} · ${e.image.width}×${e.image.height} · ${e.image.sizeBytes} bytes · ${e.image.captureMs} ms`,e.type);return;case`speech.stopped`:Q(`microphone`,`识别中…`),p_(`processing`,`正在理解`,`EVA 正在整理刚才听到的内容。`);return;case`transcript.partial`:Q(`transcript`,e.text,e.type),p_(`listening`,`我在听`,e.text);return;case`transcript.final`:Q(`transcript`,e.text,e.type),a_(t);return;case`interruption`:Q(`interruption`,`${e.reason} · ${e.turnId}`);return;case`reply.started`:Q(`reply`,`…`,e.type),p_(`processing`,`正在组织回应`,`马上就好。`);return;case`reply.partial`:$(`reply`).textContent+=e.text;return;case`reply.final`:Q(`reply`,e.text,e.type),a_(t),Eg||p_(Tg?`ready`:`idle`,Tg?`我在听`:`语音输入已关闭`,Tg?`继续说，EVA 会自然接着聊。`:`打开麦克风，或使用下方文字输入。`);return;case`playback.started`:Q(`status`,`播放回复`),p_(`speaking`,`EVA 正在说话`,`你可以随时开口打断。`);return;case`playback.stopped`:Q(`status`,`已接通`),p_(Tg?`ready`:`idle`,Tg?`我在听`:`语音输入已关闭`,Tg?`继续说，EVA 会自然接着聊。`:`打开麦克风，或使用下方文字输入。`);return;case`turn.latency`:Q(`latency`,JSON.stringify(e.latency,null,2));return;case`emotion.detected`:Q(`emotion`,`${e.emotionCode} · ${e.source} · confidence ${e.confidence?.toFixed(3)??`—`} · ${e.latencyMs} ms · 「${e.textPreview}」`,e.type);return;case`command.called`:Q(`command`,`调用中 · ${e.call.name} · ${e.call.argumentsJson}`,e.type);return;case`command.completed`:Q(`command`,`已完成 · ${e.call.name} · ${e.result.message??`成功`}${i_(e.result.data)}`,e.type);return;case`command.failed`:Q(`command`,`失败 · ${e.call.name} · ${e.result.message}${i_(e.result.data)}`,e.type);return;case`error`:e.error.source===`media`&&e.error.role===`camera`?e.error.operation===`start`&&(Dg=!1,f_(Fg,!1),Q(`camera-status`,`不可用或未授权`)):p_(`error`,`遇到了一点问题`,`请查看开发者诊断，或结束会话后重试。`),Q(`error`,JSON.stringify(e.error,null,2));return;default:return}}function i_(e){return e===void 0?``:` · ${JSON.stringify(e)}`}function a_(e){let t=$(`messages`),n=e.getMessages();Kg.hidden=n.length>0,t.replaceChildren(...n.map(e=>{let t=document.createElement(`li`);t.className=e.role;let n=document.createElement(`strong`);n.textContent=e.role===`user`?`你`:`EVA`;let r=document.createElement(`span`);return r.textContent=e.content,t.append(n,r),t}))}function o_(e){jg.disabled=e,e&&(Rg.disabled=!0)}function s_(e){Og=e,Ng.disabled=e,Pg.disabled=e,Fg.disabled=e}function c_(e){jg.hidden=e,Mg.hidden=!e,Mg.disabled=!e,e?l_():u_()}function l_(){u_(),kg=Date.now(),qg.hidden=!1,d_(),Ag=window.setInterval(d_,1e3)}function u_(){Ag!==void 0&&window.clearInterval(Ag),Ag=void 0,kg=void 0,qg.hidden=!0}function d_(){if(kg===void 0)return;let e=Math.floor((Date.now()-kg)/1e3);qg.textContent=`${Math.floor(e/60).toString().padStart(2,`0`)}:${(e%60).toString().padStart(2,`0`)}`}function f_(e,t){e.setAttribute(`aria-checked`,String(t))}function p_(e,t,n){Gg.dataset.state=e,$(`voice-state`).textContent=t,$(`voice-hint`).textContent=n}function Q(e,t,n=e){$(e).textContent=t,m_(n,t)}function m_(e,t){let n=$(`activity-log`),r=new Date().toLocaleTimeString(`zh-CN`,{hour12:!1});n.value+=`${n.value.length===0?``:`
`}[${r}] ${e}: ${t.replaceAll(`
