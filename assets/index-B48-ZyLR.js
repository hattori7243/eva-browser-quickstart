...
registerProcessor("${Bm}", EvaAudioCaptureProcessor);
`;function Um(e={}){return Wm(e)}function Wm(e={},t={}){return new Gm(e,t)}var Gm=class{constructor(e,t){this.options=e,this.dependencies=t}options;dependencies;generation=0;session;async start(){await(this.session??this.createSession()).startPromise}frames(e){let t=this.session;return t===void 0?Ym(new Q(`Browser audio input is not started`,{fatal:!0})):qm(e)?this.consumeAbortedSession(t):t.consumerActive?Ym(new Q(`Browser audio input already has an active consumer`,{fatal:!0})):(t.consumerActive=!0,this.consumeSession(t,e))}async*consumeAbortedSession(e){await this.teardown(e)}async*consumeSession(e,t){let n,r,i=new Promise(e=>{n=e}),a=()=>{r=this.teardown(e),n?.()};t?.addEventListener(`abort`,a,{once:!0});try{if(await Promise.race([e.startPromise,i]),e.failure!==void 0)throw e.failure;if(qm(t)||e.cancelled)return;for(;!qm(t);){if(e.failure!==void 0)throw e.failure;if(e.cancelled)return;let t=await e.queue.next();if(t.done)return;yield t.value}}finally{t?.removeEventListener(`abort`,a),e.consumerActive=!1,await(r??this.teardown(e))}}async stop(){let e=this.session;e!==void 0&&await this.teardown(e)}createSession(){let e,t,n=new Promise((e,n)=>{t=n}),r=new Xm(Zm(this.options.maxBufferedChunks),t=>{e.failure=t,this.teardown(e)});return e={generation:++this.generation,queue:r,encoder:new Lm,startPromise:Promise.resolve(),consumerActive:!1,cancelled:!1,rejectCancellation:t,stream:void 0,context:void 0,sourceNode:void 0,workletNode:void 0,sampleRate:void 0,ownedWorkletUrl:void 0},this.session=e,e.startPromise=Promise.race([this.startSession(e),n]),e.startPromise.catch(()=>{}),e}async startSession(e){try{let t=await $m(this.dependencies).getUserMedia({audio:{autoGainControl:this.options.autoGainControl??!0,echoCancellation:this.options.echoCancellation??!0,noiseSuppression:this.options.noiseSuppression??!0}});if(!this.isCurrent(e))throw Jm(t),Km();e.stream=t;let n=eh(this.dependencies);e.context=n,e.sampleRate=n.sampleRate,this.throwIfNotCurrent(e);let r=this.dependencies.workletModuleUrl??nh(this.dependencies).create(Hm);this.dependencies.workletModuleUrl===void 0&&(e.ownedWorkletUrl=r),await n.audioWorklet.addModule(r),this.throwIfNotCurrent(e);let i=n.createMediaStreamSource(t),a=th(this.dependencies,n);e.sourceNode=i,e.workletNode=a,a.port.onmessage=t=>this.handleSamples(e,t.data),i.connect(a),a.connect(n.destination),await n.resume(),this.throwIfNotCurrent(e)}catch(t){let n=e.cancelled||this.session!==e;throw await this.teardown(e),n?Km():Fm(t,{message:`Browser audio input failed`})}}throwIfNotCurrent(e){if(!this.isCurrent(e))throw Km()}isCurrent(e){return!e.cancelled&&this.session===e}handleSamples(e,t){if(!(!this.isCurrent(e)||!Qm(t)||e.context===void 0||e.sampleRate===void 0))try{if(e.context.sampleRate!==e.sampleRate)throw RangeError(`AudioContext sampleRate changed during capture`);for(let n of e.encoder.push(t,e.sampleRate))e.queue.push({data:n,sampleRate:e.sampleRate,channels:1,format:`pcm_s16le`})}catch(t){let n=Fm(t,{message:`Browser audio input encoding failed`});e.failure=n,e.queue.fail(n),this.teardown(e)}}teardown(e){if(e.teardownPromise!==void 0)return e.teardownPromise;e.cancelled=!0,e.rejectCancellation?.(Km()),e.rejectCancellation=void 0,e.queue.close(),this.session===e&&(this.session=void 0);let t=e.workletNode,n=e.sourceNode,r=e.stream,i=e.context,a=e.ownedWorkletUrl;return e.workletNode=void 0,e.sourceNode=void 0,e.stream=void 0,e.context=void 0,e.sampleRate=void 0,e.ownedWorkletUrl=void 0,e.teardownPromise=(async()=>{t!==void 0&&(t.port.onmessage=null,t.disconnect()),n?.disconnect(),r!==void 0&&Jm(r),await i?.close(),a!==void 0&&nh(this.dependencies).revoke(a)})(),e.teardownPromise}};function Km(){return new Q(`Browser audio input stopped`,{fatal:!0})}function qm(e){return e?.aborted===!0}function Jm(e){for(let t of e.getAudioTracks())t.stop()}async function*Ym(e){throw e}var Xm=class{constructor(e,t){this.limit=e,this.onOverflow=t}limit;onOverflow;items=[];waiters=[];closed=!1;error;push(e){if(this.closed||this.error!==void 0)return;let t=this.waiters.shift();if(t!==void 0){t.resolve({done:!1,value:e});return}if(this.items.length>=this.limit){let e=new Q(`Browser audio input buffer overflow`,{fatal:!0});this.error=e,this.items.length=0;for(let e of this.waiters.splice(0))e.reject(this.error);this.onOverflow(e);return}this.items.push(e)}fail(e){if(!(this.closed||this.error!==void 0)){this.error=e,this.items.length=0;for(let t of this.waiters.splice(0))t.reject(e)}}next(){if(this.error!==void 0)return Promise.reject(this.error);let e=this.items.shift();return e===void 0?this.closed?Promise.resolve({done:!0,value:void 0}):new Promise((e,t)=>this.waiters.push({resolve:e,reject:t})):Promise.resolve({done:!1,value:e})}close(){if(!this.closed&&(this.closed=!0,this.error===void 0))for(let e of this.waiters.splice(0))e.resolve({done:!0,value:void 0})}};function Zm(e){if(e===void 0)return Vm;if(!Number.isInteger(e)||e<=0)throw new Q(`Browser audio input maxBufferedChunks must be a positive integer`,{fatal:!0});return e}function Qm(e){return Array.isArray(e)&&e.length>0&&e.every(e=>e instanceof Float32Array)}function $m(e){let t=e.mediaDevices??globalThis.navigator?.mediaDevices;if(t===void 0)throw new Q(`Browser media devices unavailable`,{fatal:!0});return t}function eh(e){if(e.audioContextFactory!==void 0)return e.audioContextFactory();if(typeof AudioContext>`u`)throw new Q(`Browser AudioContext unavailable`,{fatal:!0});return new AudioContext({latencyHint:`interactive`})}function th(e,t){return e.audioWorkletNodeFactory===void 0?new AudioWorkletNode(t,Bm):e.audioWorkletNodeFactory(t,Bm)}function nh(e){return e.objectUrl??{create(e){return URL.createObjectURL(new Blob([e],{type:`text/javascript`}))},revoke(e){URL.revokeObjectURL(e)}}}function rh(){return ih()}function ih(e={}){return new ah(e)}var ah=class{constructor(e){this.dependencies=e}dependencies;context;scheduledTime=0;active=new Set;drainWaiters=new Set;generation=0;stopped=!1;stopPromise;async enqueue(e){if(this.stopped)throw new Q(`Browser audio output stopped`,{fatal:!0});if(e.format!==`pcm_s16le`)throw new Q(`Browser audio output requires pcm_s16le`,{fatal:!0});let t=this.generation,n;try{if(n=this.context??this.createContext(),await n.resume(),!this.isCurrent(t,n))return;let r=Math.floor(e.data.byteLength/2/e.channels);if(r===0)return;let i=n.createBuffer(1,r,e.sampleRate);oh(e,i.getChannelData(0));let a=n.createBufferSource();a.buffer=i,a.connect(n.destination),a.onended=()=>{this.active.delete(a),this.resolveDrainIfIdle()};let o=Math.max(n.currentTime,this.scheduledTime);this.active.add(a);try{a.start(o),this.scheduledTime=o+i.duration}catch(e){throw a.onended=null,this.active.delete(a),this.resolveDrainIfIdle(),e}}catch(e){if(n!==void 0&&!this.isCurrent(t,n))return;throw Fm(e,{message:`Browser audio output failed`})}}flush(){this.generation+=1;let e=this.context,t=[...this.active];this.active.clear();let n;for(let e of t){e.onended=null;try{e.stop(0)}catch(e){n??=e}}if(this.scheduledTime=e?.currentTime??0,this.resolveDrainIfIdle(),n!==void 0)throw Fm(n,{message:`Browser audio output flush failed`})}drain(){return this.active.size===0?Promise.resolve():new Promise(e=>this.drainWaiters.add(e))}stop(){return this.stopPromise??=this.stopOnce(),this.stopPromise}createContext(){if(this.dependencies.audioContextFactory!==void 0)return this.context=this.dependencies.audioContextFactory(),this.context;if(typeof AudioContext>`u`)throw new Q(`Browser AudioContext unavailable`,{fatal:!0});return this.context=new AudioContext({latencyHint:`interactive`}),this.context}resolveDrainIfIdle(){if(this.active.size===0){for(let e of this.drainWaiters)e();this.drainWaiters.clear()}}isCurrent(e,t){return!this.stopped&&e===this.generation&&t===this.context}async stopOnce(){this.stopped=!0;let e;try{this.flush()}catch(t){e=t}let t=this.context;this.context=void 0,this.scheduledTime=0;try{await t?.close()}catch(t){e??=t}if(e!==void 0)throw Fm(e,{message:`Browser audio output stop failed`})}};function oh(e,t){let n=new DataView(e.data.buffer,e.data.byteOffset,e.data.byteLength);for(let r=0;r<t.length;r+=1){let i=0;for(let t=0;t<e.channels;t+=1){let a=(r*e.channels+t)*2;i+=n.getInt16(a,!0)/32768}t[r]=i/e.channels}}function sh(e={}){return ch(e)}function ch(e={},t={}){return uh(e),new lh(e,t)}var lh=class{constructor(e,t){this.options=e,this.dependencies=t}options;dependencies;generation=0;active;pendingStart;pendingCapture;teardownPromise;start(e){if(e.aborted)return Promise.reject(yh());if(this.active!==void 0||this.pendingStart!==void 0||this.teardownPromise!==void 0)return Promise.reject(new Q(`Browser camera session already exists`,{fatal:!0}));let t=++this.generation,n=_h(),r={generation:t,cancelled:!1,cancel:n.cancel,cancellation:n.promise,underlying:Promise.resolve()};return this.pendingStart=r,r.underlying=this.acquire(r).finally(()=>{this.pendingStart===r&&(this.pendingStart=void 0)}),r.underlying.catch(()=>{}),vh(r.underlying,e,()=>this.cancelStart(r),r.cancellation)}capture(e){if(e.aborted)return Promise.reject(yh());let t=this.active;if(t===void 0)return Promise.reject(new Q(`Browser camera is not started`,{fatal:!0}));if(this.pendingCapture!==void 0)return Promise.reject(new Q(`Browser camera capture is already in progress`,{fatal:!0}));let n=_h(),r={generation:t.generation,cancelled:!1,cancel:n.cancel,cancellation:n.promise,underlying:Promise.resolve(bh())};return this.pendingCapture=r,r.underlying=this.captureFrame(t,r).finally(()=>{this.pendingCapture===r&&(this.pendingCapture=void 0)}),r.underlying.catch(()=>{}),vh(r.underlying,e,()=>this.cancelCapture(r),r.cancellation)}stop(){if(this.teardownPromise!==void 0)return this.teardownPromise;let e=this.pendingStart,t=this.pendingCapture,n=this.active;this.generation+=1,e!==void 0&&this.cancelStart(e),t!==void 0&&this.cancelCapture(t),this.active=void 0,n!==void 0&&hh(n);let r=(async()=>{await Promise.allSettled([e?.underlying??Promise.resolve(),t?.underlying??Promise.resolve()]);let n=this.active;n!==void 0&&(this.active=void 0,hh(n))})();return this.teardownPromise=r.finally(()=>{(this.teardownPromise===r||this.teardownPromise!==void 0)&&(this.teardownPromise=void 0)}),this.teardownPromise}async acquire(e){let t=await dh(this.dependencies).getUserMedia({audio:!1,video:this.options.video??!0});if(!this.isCurrentStart(e))throw gh(t),yh();let n=fh(this.dependencies);n.srcObject=t;try{if(await n.play(),await mh(this.dependencies)(n),!this.isCurrentStart(e)||n.videoWidth<=0||n.videoHeight<=0)throw yh();this.active={generation:e.generation,stream:t,video:n}}catch(e){throw n.pause(),n.srcObject=null,gh(t),e}}async captureFrame(e,t){let n=e.video.videoWidth,r=e.video.videoHeight;if(n<=0||r<=0)throw new Q(`Browser camera frame is not ready`,{fatal:!0});let i=ph(this.dependencies);i.width=n,i.height=r,i.drawImage(e.video,n,r);let a=this.options.mimeType??`image/png`,o=await i.encode(a,a===`image/jpeg`?this.options.jpegQuality:void 0);if(!this.isCurrentCapture(e,t))throw yh();let s=new Uint8Array(await o.arrayBuffer());if(!this.isCurrentCapture(e,t))throw yh();let c=o.type||a;if(s.byteLength===0||c!==`image/png`&&c!==`image/jpeg`)throw new Q(`Browser camera produced invalid image data`,{fatal:!0});return{data:s,mimeType:c,width:n,height:r}}cancelStart(e){e.cancelled||(e.cancelled=!0,e.cancel())}cancelCapture(e){e.cancelled||(e.cancelled=!0,e.cancel())}isCurrentStart(e){return this.pendingStart===e&&!e.cancelled&&e.generation===this.generation}isCurrentCapture(e,t){return this.active===e&&this.pendingCapture===t&&!t.cancelled&&t.generation===this.generation}};function uh(e){if(e.jpegQuality!==void 0&&(!Number.isFinite(e.jpegQuality)||e.jpegQuality<0||e.jpegQuality>1))throw new Q(`Browser camera jpegQuality must be between 0 and 1`,{fatal:!0})}function dh(e){let t=e.mediaDevices??navigator.mediaDevices;if(t===void 0)throw new Q(`Browser camera mediaDevices is unavailable`,{fatal:!0});return t}function fh(e){if(e.createVideo!==void 0)return e.createVideo();let t=document.createElement(`video`);return t.autoplay=!0,t.muted=!0,t.playsInline=!0,t}function ph(e){if(e.createCanvas!==void 0)return e.createCanvas();let t=document.createElement(`canvas`);return{get width(){return t.width},set width(e){t.width=e},get height(){return t.height},set height(e){t.height=e},drawImage(e,n,r){let i=t.getContext(`2d`);if(i===null)throw new Q(`Browser camera canvas 2D context is unavailable`,{fatal:!0});i.drawImage(e,0,0,n,r)},encode(e,n){return new Promise((r,i)=>{t.toBlob(e=>{e===null?i(new Q(`Browser camera image encoding failed`,{fatal:!0})):r(e)},e,n)})}}}function mh(e){return e.waitForVideoFrame===void 0?async e=>{if(e.videoWidth>0&&e.videoHeight>0)return;let t=e;await new Promise((e,n)=>{let r=()=>{a(),e()},i=()=>{a(),n(new Q(`Browser camera video failed`,{fatal:!0}))},a=()=>{t.removeEventListener(`loadeddata`,r),t.removeEventListener(`canplay`,r),t.removeEventListener(`error`,i)};t.addEventListener(`loadeddata`,r,{once:!0}),t.addEventListener(`canplay`,r,{once:!0}),t.addEventListener(`error`,i,{once:!0})})}:e.waitForVideoFrame}function hh(e){e.video.pause(),e.video.srcObject=null,gh(e.stream)}function gh(e){for(let t of e.getVideoTracks())t.stop()}function _h(){let e,t=new Promise((t,n)=>{e=()=>n(yh())});return t.catch(()=>{}),{promise:t,cancel:e}}function vh(e,t,n,r){return new Promise((i,a)=>{let o=()=>{n(),a(yh())};t.addEventListener(`abort`,o,{once:!0}),Promise.race([e,r]).then(i,a).finally(()=>{t.removeEventListener(`abort`,o)}).catch(()=>{})})}function yh(){return new DOMException(`Browser camera operation aborted`,`AbortError`)}function bh(){return{data:new Uint8Array([0]),mimeType:`image/png`,width:1,height:1}}function xh(){return Im()}async function Sh(e){let t={input:Um({maxBufferedChunks:64,echoCancellation:!0,noiseSuppression:!0,autoGainControl:!0}),output:rh(),aec:xh(),camera:sh({video:{facingMode:{ideal:`user`}},mimeType:`image/png`})},n=Am({apiKey:e.apiKey,asr:{model:`fun_asr`,sampleRate:48e3},llm:{model:`doubao-seed-2-0-mini-nothink`},tts:{model:`cosyvoice_tts`,voice:`longjielidou_v3`,sampleRate:48e3},vad:{sensitivity:.7,silenceThresholdMs:400},transports:t,history:{maxTurns:10},camera:{captureTimeoutMs:1500},systemPrompt:[`你是一个简洁、自然的语音助手。`,`只有当前用户消息实际包含图片时，才可以描述当前看到的画面。`,`当前用户消息没有图片时，不得把历史中的视觉描述说成实时观察；`,`如需引用，只能明确说明那是之前看到的内容，并说明当前没有新的画面。`].join(``),greeting:{mode:`static`,text:`你好啊`}}),r=n.onEvent(t=>e.onEvent(t,n));try{return await n.setAudioInputEnabled(e.audioInputEnabled),await n.setCameraCaptureEnabled(e.cameraEnabled),await n.setTtsEnabled(e.ttsEnabled),await n.start(),{agent:n,unsubscribe:r}}catch(e){throw r(),e}}var Ch=Jh(void 0),wh,Th,Eh,Dh=!0,Oh=!0,kh=!1,Ah=!1,jh=ag(`start`),Mh=ag(`stop`),Nh=ag(`audio-input`),Ph=ag(`tts`),Fh=ag(`camera`),Ih=ag(`text-form`),Lh=ag(`text`),Rh=Ih.querySelector(`button[type=submit]`),zh=ag(`clear-log`),Bh=ag(`api-key-dialog`),Vh=ag(`api-key-form`),Hh=ag(`api-key-input`),Uh=ag(`api-key-error`),Wh=ag(`api-key-cancel`);jh.addEventListener(`click`,()=>void Gh()),Mh.addEventListener(`click`,()=>void Yh()),Nh.addEventListener(`click`,()=>void Xh()),Ph.addEventListener(`click`,()=>void Zh()),Fh.addEventListener(`click`,()=>void Qh()),Ih.addEventListener(`submit`,e=>void $h(e)),zh.addEventListener(`click`,()=>{ag(`activity-log`).value=``}),Vh.addEventListener(`submit`,e=>{if(e.preventDefault(),Jh(Hh.value)===void 0){Uh.textContent=`请输入有效的 Gateway AK`,Hh.focus();return}Bh.close(`confirm`)}),Wh.addEventListener(`click`,()=>Bh.close(`cancel`));async function Gh(){if(Th===void 0){ng(!0);try{let e=await Kh();if(e===void 0){$(`status`,`未启动；未提供 AK`);return}rg(!0),$(`status`,`启动中…`),$(`microphone`,Dh?`请求权限中…`:`已禁用`);let t=await Sh({apiKey:e,audioInputEnabled:Dh,ttsEnabled:Oh,cameraEnabled:kh,onEvent:eg});Th=t.agent,Eh=t.unsubscribe,$(`status`,`运行中`),$(`microphone`,Dh?`已连接`:`已禁用`),$(`camera-status`,kh?`持续连接；说话时采一张图`:`默认关闭`),Mh.disabled=!1,Rh.disabled=!1,rg(!1)}catch(e){$(`status`,`启动失败`),$(`microphone`,`不可用`),$(`error`,e instanceof Error?e.message:String(e)),Eh?.(),Eh=void 0,Th=void 0,rg(!1)}finally{ng(!1),jh.disabled=Th!==void 0}}}async function Kh(){if(Ch!==void 0)return Ch;if(wh!==void 0)return wh;let e=await qh();return e!==void 0&&(wh=e),e}function qh(){return Hh.value=``,Uh.textContent=``,Bh.returnValue=``,new Promise(e=>{Bh.addEventListener(`close`,()=>{let t=Bh.returnValue===`confirm`?Jh(Hh.value):void 0;Hh.value=``,e(t)},{once:!0}),Bh.showModal(),Hh.focus()})}function Jh(e){let t=e?.trim();return t===void 0||t.length===0||t.startsWith(`replace-`)?void 0:t}async function Yh(){let e=Th;if(e!==void 0){Th=void 0,Rh.disabled=!0,Mh.disabled=!0,jh.disabled=!0,rg(!0),$(`status`,`停止中…`);try{await e.stop(),tg(e),$(`status`,`已停止；再次启动会创建新会话`),$(`microphone`,`已释放`),$(`camera-status`,`已释放`)}catch(e){$(`error`,e instanceof Error?e.message:String(e))}finally{Eh?.(),Eh=void 0,jh.disabled=!1}}}async function Xh(){if(Nh.disabled)return;let e=!Dh;Nh.disabled=!0;try{await Th?.setAudioInputEnabled(e),Dh=e,Nh.textContent=`麦克风：${Dh?`开启`:`关闭`}`,$(`microphone`,Dh?Th===void 0?`启动时启用`:`已连接`:Th===void 0?`启动时禁用`:`已释放`)}catch(e){$(`error`,e instanceof Error?e.message:String(e))}finally{Nh.disabled=Ah}}async function Zh(){if(Ph.disabled)return;let e=!Oh;Ph.disabled=!0;try{await Th?.setTtsEnabled(e),Oh=e,Ph.textContent=`TTS：${Oh?`开启`:`关闭`}`}catch(e){$(`error`,e instanceof Error?e.message:String(e))}finally{Ph.disabled=Ah}}async function Qh(){if(Fh.disabled)return;let e=!kh;Fh.disabled=!0;try{await Th?.setCameraCaptureEnabled(e),kh=e,Fh.textContent=`摄像头：${kh?`开启`:`关闭`}`,$(`camera-status`,kh?Th===void 0?`启动时启用`:`持续连接；说话时采一张图`:Th===void 0?`启动时关闭`:`已释放`)}catch(e){$(`error`,e instanceof Error?e.message:String(e))}finally{Fh.disabled=Ah}}async function $h(e){e.preventDefault();let t=Lh.value.trim(),n=Th;if(!(n===void 0||t.length===0)){Lh.value=``;try{await n.submitText(t,{metadata:{channel:`quickstart`}})}catch(e){$(`error`,e instanceof Error?e.message:String(e))}}}function eg(e,t){switch(e.type){case`speech.started`:$(`microphone`,`检测到说话`);return;case`image.captured`:$(`camera-status`,`${e.image.mimeType} · ${e.image.width}×${e.image.height} · ${e.image.sizeBytes} bytes · ${e.image.captureMs} ms`,e.type);return;case`speech.stopped`:$(`microphone`,`识别中…`);return;case`transcript.partial`:$(`transcript`,e.text,e.type);return;case`transcript.final`:$(`transcript`,e.text,e.type),tg(t);return;case`interruption`:$(`interruption`,`${e.reason} · ${e.turnId}`);return;case`reply.started`:$(`reply`,`…`,e.type);return;case`reply.partial`:ag(`reply`).textContent+=e.text;return;case`reply.final`:$(`reply`,e.text,e.type),tg(t);return;case`playback.started`:$(`status`,`播放回复`);return;case`playback.stopped`:$(`status`,`运行中`);return;case`turn.latency`:$(`latency`,JSON.stringify(e.latency,null,2));return;case`error`:e.error.source===`media`&&e.error.role===`camera`&&e.error.operation===`start`&&(kh=!1,Fh.textContent=`摄像头：关闭`,$(`camera-status`,`不可用或未授权`)),$(`error`,JSON.stringify(e.error,null,2));return;default:return}}function tg(e){ag(`messages`).replaceChildren(...e.getMessages().map(e=>{let t=document.createElement(`li`);t.className=e.role;let n=document.createElement(`strong`);n.textContent=e.role===`user`?`你`:`Eva`;let r=document.createElement(`span`);return r.textContent=e.content,t.append(n,r),t}))}function ng(e){jh.disabled=e,e&&(Rh.disabled=!0)}function rg(e){Ah=e,Nh.disabled=e,Ph.disabled=e,Fh.disabled=e}function $(e,t,n=e){ag(e).textContent=t,ig(n,t)}function ig(e,t){let n=ag(`activity-log`),r=new Date().toLocaleTimeString(`zh-CN`,{hour12:!1});n.value+=`${n.value.length===0?``:`
`}[${r}] ${e}: ${t.replaceAll(`
`,` `)}`,n.scrollTop=n.scrollHeight}function ag(e){let t=document.getElementById(e);if(t===null)throw Error(`Missing #${e}`);return t}export{o as t};