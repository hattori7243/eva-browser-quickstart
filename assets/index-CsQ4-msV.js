...
registerProcessor("${Bh}", EvaAudioCaptureProcessor);
`;function Uh(e={}){return Wh(e)}function Wh(e={},t={}){return new Gh(e,t)}var Gh=class{constructor(e,t){this.options=e,this.dependencies=t}options;dependencies;generation=0;session;async start(){await(this.session??this.createSession()).startPromise}frames(e){let t=this.session;return t===void 0?Yh(new Ph(`Browser audio input is not started`,{fatal:!0})):qh(e)?this.consumeAbortedSession(t):t.consumerActive?Yh(new Ph(`Browser audio input already has an active consumer`,{fatal:!0})):(t.consumerActive=!0,this.consumeSession(t,e))}async*consumeAbortedSession(e){await this.teardown(e)}async*consumeSession(e,t){let n,r,i=new Promise(e=>{n=e}),a=()=>{r=this.teardown(e),n?.()};t?.addEventListener(`abort`,a,{once:!0});try{await Promise.race([e.startPromise,i]);let n=e.failure;if(n!==void 0)throw n;if(qh(t)||e.cancelled)return;for(;!qh(t);){let t=e.failure;if(t!==void 0)throw t;if(e.cancelled)return;let n=await e.queue.next();if(n.done)return;yield n.value}}finally{t?.removeEventListener(`abort`,a),e.consumerActive=!1,await(r??this.teardown(e))}}async stop(){let e=this.session;e!==void 0&&await this.teardown(e)}createSession(){let e,t,n=new Promise((e,n)=>{t=n}),r=new Xh(Zh(this.options.maxBufferedChunks),t=>{e.failure=t,this.teardown(e)});return e={generation:++this.generation,queue:r,encoder:new Lh,startPromise:Promise.resolve(),consumerActive:!1,cancelled:!1,rejectCancellation:t,stream:void 0,context:void 0,sourceNode:void 0,workletNode:void 0,sampleRate:void 0,ownedWorkletUrl:void 0},this.session=e,e.startPromise=Promise.race([this.startSession(e),n]),e.startPromise.catch(()=>{}),e}async startSession(e){try{let t=await $h(this.dependencies).getUserMedia({audio:{autoGainControl:this.options.autoGainControl??!0,echoCancellation:this.options.echoCancellation??!0,noiseSuppression:this.options.noiseSuppression??!0}});if(!this.isCurrent(e))throw Jh(t),Kh();e.stream=t;let n=eg(this.dependencies);e.context=n,e.sampleRate=n.sampleRate,this.throwIfNotCurrent(e);let r=this.dependencies.workletModuleUrl??ng(this.dependencies).create(Hh);this.dependencies.workletModuleUrl===void 0&&(e.ownedWorkletUrl=r),await n.audioWorklet.addModule(r),this.throwIfNotCurrent(e);let i=n.createMediaStreamSource(t),a=tg(this.dependencies,n);e.sourceNode=i,e.workletNode=a,a.port.onmessage=t=>this.handleSamples(e,t.data),i.connect(a),a.connect(n.destination),await n.resume(),this.throwIfNotCurrent(e)}catch(t){let n=e.cancelled||this.session!==e;throw await this.teardown(e),n?Kh():Fh(t,{message:`Browser audio input failed`})}}throwIfNotCurrent(e){if(!this.isCurrent(e))throw Kh()}isCurrent(e){return!e.cancelled&&this.session===e}handleSamples(e,t){if(!(!this.isCurrent(e)||!Qh(t)||e.context===void 0||e.sampleRate===void 0))try{if(e.context.sampleRate!==e.sampleRate)throw RangeError(`AudioContext sampleRate changed during capture`);for(let n of e.encoder.push(t,e.sampleRate))e.queue.push({data:n,sampleRate:e.sampleRate,channels:1,format:`pcm_s16le`})}catch(t){let n=Fh(t,{message:`Browser audio input encoding failed`});e.failure=n,e.queue.fail(n),this.teardown(e)}}teardown(e){if(e.teardownPromise!==void 0)return e.teardownPromise;e.cancelled=!0,e.rejectCancellation?.(Kh()),e.rejectCancellation=void 0,e.queue.close(),this.session===e&&(this.session=void 0);let t=e.workletNode,n=e.sourceNode,r=e.stream,i=e.context,a=e.ownedWorkletUrl;return e.workletNode=void 0,e.sourceNode=void 0,e.stream=void 0,e.context=void 0,e.sampleRate=void 0,e.ownedWorkletUrl=void 0,e.teardownPromise=(async()=>{let e;try{t!==void 0&&(t.port.onmessage=null,t.disconnect())}catch(t){e=t}try{n?.disconnect()}catch(t){e??=t}try{r!==void 0&&Jh(r)}catch(t){e??=t}try{await i?.close()}catch(t){e??=t}try{a!==void 0&&ng(this.dependencies).revoke(a)}catch(t){e??=t}if(e!==void 0)throw Fh(e,{message:`Browser audio input stop failed`})})(),e.teardownPromise}};function Kh(){return new Ph(`Browser audio input stopped`,{fatal:!0})}function qh(e){return e?.aborted===!0}function Jh(e){for(let t of e.getAudioTracks())t.stop()}async function*Yh(e){throw e}var Xh=class{constructor(e,t){this.limit=e,this.onOverflow=t}limit;onOverflow;items=[];waiters=[];closed=!1;error;push(e){if(this.closed||this.error!==void 0)return;let t=this.waiters.shift();if(t!==void 0){t.resolve({done:!1,value:e});return}if(this.items.length>=this.limit){let e=new Ph(`Browser audio input buffer overflow`,{fatal:!0});this.error=e,this.items.length=0;for(let e of this.waiters.splice(0))e.reject(this.error);this.onOverflow(e);return}this.items.push(e)}fail(e){if(!(this.closed||this.error!==void 0)){this.error=e,this.items.length=0;for(let t of this.waiters.splice(0))t.reject(e)}}next(){if(this.error!==void 0)return Promise.reject(this.error);let e=this.items.shift();return e===void 0?this.closed?Promise.resolve({done:!0,value:void 0}):new Promise((e,t)=>this.waiters.push({resolve:e,reject:t})):Promise.resolve({done:!1,value:e})}close(){if(!this.closed&&(this.closed=!0,this.error===void 0))for(let e of this.waiters.splice(0))e.resolve({done:!0,value:void 0})}};function Zh(e){if(e===void 0)return Vh;if(!Number.isInteger(e)||e<=0)throw new Ph(`Browser audio input maxBufferedChunks must be a positive integer`,{fatal:!0});return e}function Qh(e){return Array.isArray(e)&&e.length>0&&e.every(e=>e instanceof Float32Array)}function $h(e){let t=e.mediaDevices??globalThis.navigator?.mediaDevices;if(t===void 0)throw new Ph(`Browser media devices unavailable`,{fatal:!0});return t}function eg(e){if(e.audioContextFactory!==void 0)return e.audioContextFactory();if(typeof AudioContext>`u`)throw new Ph(`Browser AudioContext unavailable`,{fatal:!0});return new AudioContext({latencyHint:`interactive`})}function tg(e,t){return e.audioWorkletNodeFactory===void 0?new AudioWorkletNode(t,Bh):e.audioWorkletNodeFactory(t,Bh)}function ng(e){return e.objectUrl??{create(e){return URL.createObjectURL(new Blob([e],{type:`text/javascript`}))},revoke(e){URL.revokeObjectURL(e)}}}function rg(){return ig()}function ig(e={}){return new ag(e)}var ag=class{constructor(e){this.dependencies=e}dependencies;context;scheduledTime=0;active=new Set;drainWaiters=new Set;generation=0;stopped=!1;stopPromise;async enqueue(e){if(this.stopped)throw new Ph(`Browser audio output stopped`,{fatal:!0});if(e.format!==`pcm_s16le`)throw new Ph(`Browser audio output requires pcm_s16le`,{fatal:!0});let t=this.generation,n;try{if(n=this.context??this.createContext(),await n.resume(),!this.isCurrent(t,n))return;let r=Math.floor(e.data.byteLength/2/e.channels);if(r===0)return;let i=n.createBuffer(1,r,e.sampleRate);og(e,i.getChannelData(0));let a=n.createBufferSource();a.buffer=i,a.connect(n.destination),a.onended=()=>{this.active.delete(a),this.resolveDrainIfIdle()};let o=Math.max(n.currentTime,this.scheduledTime);this.active.add(a);try{a.start(o),this.scheduledTime=o+i.duration}catch(e){throw a.onended=null,this.active.delete(a),this.resolveDrainIfIdle(),e}}catch(e){if(n!==void 0&&!this.isCurrent(t,n))return;throw Fh(e,{message:`Browser audio output failed`})}}flush(){this.generation+=1;let e=this.context,t=[...this.active];this.active.clear();let n;for(let e of t){e.onended=null;try{e.stop(0)}catch(e){n??=e}}if(this.scheduledTime=e?.currentTime??0,this.resolveDrainIfIdle(),n!==void 0)throw Fh(n,{message:`Browser audio output flush failed`})}drain(){return this.active.size===0?Promise.resolve():new Promise(e=>this.drainWaiters.add(e))}stop(){return this.stopPromise??=this.stopOnce(),this.stopPromise}createContext(){if(this.dependencies.audioContextFactory!==void 0)return this.context=this.dependencies.audioContextFactory(),this.context;if(typeof AudioContext>`u`)throw new Ph(`Browser AudioContext unavailable`,{fatal:!0});return this.context=new AudioContext({latencyHint:`interactive`}),this.context}resolveDrainIfIdle(){if(this.active.size===0){for(let e of this.drainWaiters)e();this.drainWaiters.clear()}}isCurrent(e,t){return!this.stopped&&e===this.generation&&t===this.context}async stopOnce(){this.stopped=!0;let e;try{this.flush()}catch(t){e=t}let t=this.context;this.context=void 0,this.scheduledTime=0;try{await t?.close()}catch(t){e??=t}if(e!==void 0)throw Fh(e,{message:`Browser audio output stop failed`})}};function og(e,t){let n=new DataView(e.data.buffer,e.data.byteOffset,e.data.byteLength);for(let r=0;r<t.length;r+=1){let i=0;for(let t=0;t<e.channels;t+=1){let a=(r*e.channels+t)*2;i+=n.getInt16(a,!0)/32768}t[r]=i/e.channels}}function sg(e={}){return cg(e)}function cg(e={},t={}){return ug(e),new lg(e,t)}var lg=class{constructor(e,t){this.options=e,this.dependencies=t}options;dependencies;generation=0;active;pendingStart;pendingCapture;teardownPromise;start(e){if(e.aborted)return Promise.reject(yg());if(this.active!==void 0||this.pendingStart!==void 0||this.teardownPromise!==void 0)return Promise.reject(new Ph(`Browser camera session already exists`,{fatal:!0}));let t=++this.generation,n=_g(),r={generation:t,cancelled:!1,cancel:n.cancel,cancellation:n.promise,underlying:Promise.resolve()};return this.pendingStart=r,r.underlying=this.acquire(r).finally(()=>{this.pendingStart===r&&(this.pendingStart=void 0)}),r.underlying.catch(()=>{}),vg(r.underlying,e,()=>this.cancelStart(r),r.cancellation)}capture(e){if(e.aborted)return Promise.reject(yg());let t=this.active;if(t===void 0)return Promise.reject(new Ph(`Browser camera is not started`,{fatal:!0}));if(this.pendingCapture!==void 0)return Promise.reject(new Ph(`Browser camera capture is already in progress`,{fatal:!0}));let n=_g(),r={generation:t.generation,cancelled:!1,cancel:n.cancel,cancellation:n.promise,underlying:Promise.resolve(bg())};return this.pendingCapture=r,r.underlying=this.captureFrame(t,r).finally(()=>{this.pendingCapture===r&&(this.pendingCapture=void 0)}),r.underlying.catch(()=>{}),vg(r.underlying,e,()=>this.cancelCapture(r),r.cancellation)}stop(){if(this.teardownPromise!==void 0)return this.teardownPromise;let e=this.pendingStart,t=this.pendingCapture,n=this.active;this.generation+=1,e!==void 0&&this.cancelStart(e),t!==void 0&&this.cancelCapture(t),this.active=void 0,n!==void 0&&hg(n);let r=(async()=>{await Promise.allSettled([e?.underlying??Promise.resolve(),t?.underlying??Promise.resolve()]);let n=this.active;n!==void 0&&(this.active=void 0,hg(n))})();return this.teardownPromise=r.finally(()=>{(this.teardownPromise===r||this.teardownPromise!==void 0)&&(this.teardownPromise=void 0)}),this.teardownPromise}async acquire(e){let t=await dg(this.dependencies).getUserMedia({audio:!1,video:this.options.video??!0});if(!this.isCurrentStart(e))throw gg(t),yg();let n=fg(this.dependencies);n.srcObject=t;try{if(await n.play(),await mg(this.dependencies)(n),!this.isCurrentStart(e)||n.videoWidth<=0||n.videoHeight<=0)throw yg();this.active={generation:e.generation,stream:t,video:n}}catch(e){throw n.pause(),n.srcObject=null,gg(t),e}}async captureFrame(e,t){let n=e.video.videoWidth,r=e.video.videoHeight;if(n<=0||r<=0)throw new Ph(`Browser camera frame is not ready`,{fatal:!0});let i=pg(this.dependencies);i.width=n,i.height=r,i.drawImage(e.video,n,r);let a=this.options.mimeType??`image/png`,o=await i.encode(a,a===`image/jpeg`?this.options.jpegQuality:void 0);if(!this.isCurrentCapture(e,t))throw yg();let s=new Uint8Array(await o.arrayBuffer());if(!this.isCurrentCapture(e,t))throw yg();let c=o.type||a;if(s.byteLength===0||c!==`image/png`&&c!==`image/jpeg`)throw new Ph(`Browser camera produced invalid image data`,{fatal:!0});return{data:s,mimeType:c,width:n,height:r}}cancelStart(e){e.cancelled||(e.cancelled=!0,e.cancel())}cancelCapture(e){e.cancelled||(e.cancelled=!0,e.cancel())}isCurrentStart(e){return this.pendingStart===e&&!e.cancelled&&e.generation===this.generation}isCurrentCapture(e,t){return this.active===e&&this.pendingCapture===t&&!t.cancelled&&t.generation===this.generation}};function ug(e){if(e.jpegQuality!==void 0&&(!Number.isFinite(e.jpegQuality)||e.jpegQuality<0||e.jpegQuality>1))throw new Ph(`Browser camera jpegQuality must be between 0 and 1`,{fatal:!0})}function dg(e){let t=e.mediaDevices??navigator.mediaDevices;if(t===void 0)throw new Ph(`Browser camera mediaDevices is unavailable`,{fatal:!0});return t}function fg(e){if(e.createVideo!==void 0)return e.createVideo();let t=document.createElement(`video`);return t.autoplay=!0,t.muted=!0,t.playsInline=!0,t}function pg(e){if(e.createCanvas!==void 0)return e.createCanvas();let t=document.createElement(`canvas`);return{get width(){return t.width},set width(e){t.width=e},get height(){return t.height},set height(e){t.height=e},drawImage(e,n,r){let i=t.getContext(`2d`);if(i===null)throw new Ph(`Browser camera canvas 2D context is unavailable`,{fatal:!0});i.drawImage(e,0,0,n,r)},encode(e,n){return new Promise((r,i)=>{t.toBlob(e=>{e===null?i(new Ph(`Browser camera image encoding failed`,{fatal:!0})):r(e)},e,n)})}}}function mg(e){return e.waitForVideoFrame===void 0?async e=>{if(e.videoWidth>0&&e.videoHeight>0)return;let t=e;await new Promise((e,n)=>{let r=()=>{a(),e()},i=()=>{a(),n(new Ph(`Browser camera video failed`,{fatal:!0}))},a=()=>{t.removeEventListener(`loadeddata`,r),t.removeEventListener(`canplay`,r),t.removeEventListener(`error`,i)};t.addEventListener(`loadeddata`,r,{once:!0}),t.addEventListener(`canplay`,r,{once:!0}),t.addEventListener(`error`,i,{once:!0})})}:e.waitForVideoFrame}function hg(e){e.video.pause(),e.video.srcObject=null,gg(e.stream)}function gg(e){for(let t of e.getVideoTracks())t.stop()}function _g(){let e,t=new Promise((t,n)=>{e=()=>n(yg())});return t.catch(()=>{}),{promise:t,cancel:e}}function vg(e,t,n,r){return new Promise((i,a)=>{let o=()=>{n(),a(yg())};t.addEventListener(`abort`,o,{once:!0}),Promise.race([e,r]).then(i,a).finally(()=>{t.removeEventListener(`abort`,o)}).catch(()=>{})})}function yg(){return new DOMException(`Browser camera operation aborted`,`AbortError`)}function bg(){return{data:new Uint8Array([0]),mimeType:`image/png`,width:1,height:1}}function xg(){return Ih()}var Sg=[{definition:{name:`show_current_time`,description:`当用户询问当前时间时，查询当前页面所在设备的本地时间。`},handler(e,t){if(t.signal.aborted)return{ok:!1,message:`Command cancelled`};let n=new Date;return{ok:!0,message:`当前本地时间是 ${n.toLocaleTimeString()}`,data:{isoTime:n.toISOString(),localTime:n.toLocaleString(),timeZone:Intl.DateTimeFormat().resolvedOptions().timeZone}}}},{definition:{name:`set_page_theme`,description:`当用户要求切换页面外观时，将当前页面切换为指定的明暗主题。`,parameters:[{name:`theme`,description:`要应用的页面主题。`,type:`string`,required:!0,enum:[`light`,`dark`],example:`dark`}]},handler(e,t){if(t.signal.aborted)return{ok:!1,message:`Command cancelled`};let n=e.arguments.theme;return n!==`light`&&n!==`dark`?{ok:!1,message:`Unsupported page theme`}:(Cg(n),{ok:!0,message:`页面主题已切换为 ${n}`,data:{theme:n}})}}];function Cg(e){document.documentElement.dataset.theme=e,document.documentElement.style.colorScheme=e}async function wg(e){let t={input:Uh({maxBufferedChunks:64,echoCancellation:!0,noiseSuppression:!0,autoGainControl:!0}),output:rg(),aec:xg(),camera:sg({video:{facingMode:{ideal:`user`}},mimeType:`image/png`})},n=kh({apiKey:e.apiKey,asr:{model:`fun_asr`,sampleRate:48e3},llm:{model:`doubao-seed-2-0-mini-nothink`},tts:{model:`cosyvoice_tts`,voice:`longjielidou_v3`,sampleRate:48e3},vad:{sensitivity:.7,silenceThresholdMs:400},transports:t,history:{maxTurns:10},camera:{captureTimeoutMs:1500},emotion:{enabled:!0,labels:[`happy`,`sad`]},commands:{registrations:Sg,maxCallsPerTurn:3},systemPrompt:[`你是一个简洁、自然的语音助手。`,`只有当前用户消息实际包含图片时，才可以描述当前看到的画面。`,`当前用户消息没有图片时，不得把历史中的视觉描述说成实时观察；`,`如需引用，只能明确说明那是之前看到的内容，并说明当前没有新的画面。`].join(``),greeting:{mode:`static`,text:`你好`}}),r=n.onEvent(t=>e.onEvent(t,n));try{return await n.setAudioInputEnabled(e.audioInputEnabled),await n.setCameraCaptureEnabled(e.cameraEnabled),await n.setTtsEnabled(e.ttsEnabled),await n.start(),{agent:n,unsubscribe:r}}catch(e){throw r(),e}}var Tg=t_(void 0),Eg,Dg,Og,kg=!0,Ag=!0,jg=!1,Mg=!1,Ng,Pg,Fg=$(`start`),Ig=$(`stop`),Lg=$(`audio-input`),Rg=$(`tts`),zg=$(`camera`),Bg=$(`text-form`),Vg=$(`text`),Hg=Bg.querySelector(`button[type=submit]`),Ug=$(`clear-log`),Wg=$(`api-key-dialog`),Gg=$(`api-key-form`),Kg=$(`api-key-input`),qg=$(`api-key-error`),Jg=$(`api-key-cancel`),Yg=$(`voice-stage`),Xg=$(`conversation-empty`),Zg=$(`call-duration`);Fg.addEventListener(`click`,()=>void Qg()),Ig.addEventListener(`click`,()=>void n_()),Lg.addEventListener(`click`,()=>void r_()),Rg.addEventListener(`click`,()=>void i_()),zg.addEventListener(`click`,()=>void a_()),Bg.addEventListener(`submit`,e=>void o_(e)),Ug.addEventListener(`click`,()=>{$(`activity-log`).value=``}),Gg.addEventListener(`submit`,e=>{if(e.preventDefault(),t_(Kg.value)===void 0){qg.textContent=`请输入有效的 Gateway AK`,Kg.focus();return}Wg.close(`confirm`)}),Jg.addEventListener(`click`,()=>Wg.close(`cancel`));async function Qg(){if(Dg===void 0){u_(!0);try{let e=await $g();if(e===void 0){Q(`status`,`未启动；未提供 AK`),__(`idle`,`准备好聊聊了吗？`,`启动后直接开口，EVA 会实时听见并回应你。`);return}d_(!0),Q(`status`,`启动中…`),__(`starting`,`正在连接 EVA`,`请稍候，我们正在准备麦克风和语音通道。`),Q(`microphone`,kg?`请求权限中…`:`已禁用`);let t=await wg({apiKey:e,audioInputEnabled:kg,ttsEnabled:Ag,cameraEnabled:jg,onEvent:s_});Dg=t.agent,Og=t.unsubscribe,Q(`status`,`已接通`),__(kg?`ready`:`idle`,kg?`通话已接通`:`语音输入已关闭`,kg?`直接开口即可，EVA 在听。`:`打开麦克风，或使用下方文字输入。`),Q(`microphone`,kg?`已连接`:`已禁用`),Q(`camera-status`,jg?`持续连接；说话时采一张图`:`默认关闭`),f_(!0),Hg.disabled=!1,d_(!1)}catch(e){Q(`status`,`启动失败`),__(`error`,`连接没有成功`,`请检查 AK、网络与设备权限后重试。`),Q(`microphone`,`不可用`),Q(`error`,e instanceof Error?e.message:String(e)),Og?.(),Og=void 0,Dg=void 0,f_(!1),d_(!1)}finally{u_(!1),Fg.disabled=Dg!==void 0}}}async function $g(){if(Tg!==void 0)return Tg;if(Eg!==void 0)return Eg;let e=await e_();return e!==void 0&&(Eg=e),e}function e_(){return Kg.value=``,qg.textContent=``,Wg.returnValue=``,new Promise(e=>{Wg.addEventListener(`close`,()=>{let t=Wg.returnValue===`confirm`?t_(Kg.value):void 0;Kg.value=``,e(t)},{once:!0}),Wg.showModal(),Kg.focus()})}function t_(e){let t=e?.trim();return t===void 0||t.length===0||t.startsWith(`replace-`)?void 0:t}async function n_(){let e=Dg;if(e!==void 0){Dg=void 0,Hg.disabled=!0,Ig.disabled=!0,Fg.disabled=!0,m_(),d_(!0),Q(`status`,`停止中…`),__(`processing`,`正在结束会话`,`EVA 正在释放本次会话使用的设备。`);try{await e.stop(),l_(e),Q(`status`,`通话已结束；再次启动会创建新会话`),__(`idle`,`本次对话已结束`,`再次启动会创建一个新的会话。`),Q(`microphone`,`已释放`),Q(`camera-status`,`已释放`)}catch(e){Q(`error`,e instanceof Error?e.message:String(e))}finally{Og?.(),Og=void 0,f_(!1),Fg.disabled=!1}}}async function r_(){if(Lg.disabled)return;let e=!kg;Lg.disabled=!0;try{await Dg?.setAudioInputEnabled(e),kg=e,g_(Lg,kg),Dg!==void 0&&__(kg?`ready`:`idle`,kg?`我在听`:`语音输入已关闭`,kg?`继续说，EVA 会自然接着聊。`:`打开麦克风，或使用下方文字输入。`),Q(`microphone`,kg?Dg===void 0?`启动时启用`:`已连接`:Dg===void 0?`启动时禁用`:`已释放`)}catch(e){Q(`error`,e instanceof Error?e.message:String(e))}finally{Lg.disabled=Mg}}async function i_(){if(Rg.disabled)return;let e=!Ag;Rg.disabled=!0;try{await Dg?.setTtsEnabled(e),Ag=e,g_(Rg,Ag)}catch(e){Q(`error`,e instanceof Error?e.message:String(e))}finally{Rg.disabled=Mg}}async function a_(){if(zg.disabled)return;let e=!jg;zg.disabled=!0;try{await Dg?.setCameraCaptureEnabled(e),jg=e,g_(zg,jg),Q(`camera-status`,jg?Dg===void 0?`启动时启用`:`持续连接；说话时采一张图`:Dg===void 0?`启动时关闭`:`已释放`)}catch(e){Q(`error`,e instanceof Error?e.message:String(e))}finally{zg.disabled=Mg}}async function o_(e){e.preventDefault();let t=Vg.value.trim(),n=Dg;if(!(n===void 0||t.length===0)){Vg.value=``;try{await n.submitText(t,{metadata:{channel:`ts-browser-demo`}})}catch(e){Q(`error`,e instanceof Error?e.message:String(e))}}}function s_(e,t){switch(e.type){case`speech.started`:Q(`microphone`,`检测到说话`),__(`listening`,`我在听`,`继续说，停顿后 EVA 会自动理解并回应。`);return;case`image.captured`:Q(`camera-status`,`${e.image.mimeType} · ${e.image.width}×${e.image.height} · ${e.image.sizeBytes} bytes · ${e.image.captureMs} ms`,e.type);return;case`speech.stopped`:Q(`microphone`,`识别中…`),__(`processing`,`正在理解`,`EVA 正在整理刚才听到的内容。`);return;case`transcript.partial`:Q(`transcript`,e.text,e.type),__(`listening`,`我在听`,e.text);return;case`transcript.final`:Q(`transcript`,e.text,e.type),l_(t);return;case`interruption`:Q(`interruption`,`${e.reason} · ${e.turnId}`);return;case`reply.started`:Q(`reply`,`…`,e.type),__(`processing`,`正在组织回应`,`马上就好。`);return;case`reply.partial`:$(`reply`).textContent+=e.text;return;case`reply.final`:Q(`reply`,e.text,e.type),l_(t),Ag||__(kg?`ready`:`idle`,kg?`我在听`:`语音输入已关闭`,kg?`继续说，EVA 会自然接着聊。`:`打开麦克风，或使用下方文字输入。`);return;case`playback.started`:Q(`status`,`播放回复`),__(`speaking`,`EVA 正在说话`,`你可以随时开口打断。`);return;case`playback.stopped`:Q(`status`,`已接通`),__(kg?`ready`:`idle`,kg?`我在听`:`语音输入已关闭`,kg?`继续说，EVA 会自然接着聊。`:`打开麦克风，或使用下方文字输入。`);return;case`turn.latency`:Q(`latency`,JSON.stringify(e.latency,null,2));return;case`emotion.detected`:Q(`emotion`,`${e.emotionCode} · ${e.source} · confidence ${e.confidence?.toFixed(3)??`—`} · ${e.latencyMs} ms · 「${e.textPreview}」`,e.type);return;case`command.called`:Q(`command`,`调用中 · ${e.call.name} · ${e.call.argumentsJson}`,e.type);return;case`command.completed`:Q(`command`,`已完成 · ${e.call.name} · ${e.result.message??`成功`}${c_(e.result.data)}`,e.type);return;case`command.failed`:Q(`command`,`失败 · ${e.call.name} · ${e.result.message}${c_(e.result.data)}`,e.type);return;case`error`:e.error.source===`media`&&e.error.role===`camera`?e.error.operation===`start`&&(jg=!1,g_(zg,!1),Q(`camera-status`,`不可用或未授权`)):__(`error`,`遇到了一点问题`,`请查看开发者诊断，或结束会话后重试。`),Q(`error`,JSON.stringify(e.error,null,2));return;default:return}}function c_(e){return e===void 0?``:` · ${JSON.stringify(e)}`}function l_(e){let t=$(`messages`),n=e.getMessages();Xg.hidden=n.length>0,t.replaceChildren(...n.map(e=>{let t=document.createElement(`li`);t.className=e.role;let n=document.createElement(`strong`);n.textContent=e.role===`user`?`你`:`EVA`;let r=document.createElement(`span`);return r.textContent=e.content,t.append(n,r),t}))}function u_(e){Fg.disabled=e,e&&(Hg.disabled=!0)}function d_(e){Mg=e,Lg.disabled=e,Rg.disabled=e,zg.disabled=e}function f_(e){Fg.hidden=e,Ig.hidden=!e,Ig.disabled=!e,e?p_():m_()}function p_(){m_(),Ng=Date.now(),Zg.hidden=!1,h_(),Pg=window.setInterval(h_,1e3)}function m_(){Pg!==void 0&&window.clearInterval(Pg),Pg=void 0,Ng=void 0,Zg.hidden=!0}function h_(){if(Ng===void 0)return;let e=Math.floor((Date.now()-Ng)/1e3);Zg.textContent=`${Math.floor(e/60).toString().padStart(2,`0`)}:${(e%60).toString().padStart(2,`0`)}`}function g_(e,t){e.setAttribute(`aria-checked`,String(t))}function __(e,t,n){Yg.dataset.state=e,$(`voice-state`).textContent=t,$(`voice-hint`).textContent=n}function Q(e,t,n=e){$(e).textContent=t,v_(n,t)}function v_(e,t){let n=$(`activity-log`),r=new Date().toLocaleTimeString(`zh-CN`,{hour12:!1});n.value+=`${n.value.length===0?``:`
`}[${r}] ${e}: ${t.replaceAll(`
